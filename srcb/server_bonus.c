...
/*   Updated: 2022/09/11 12:28:28 by sciftci          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	msg_received(t_protocol *t_server, pid_t client_pid)
{
	...
	{
		t_server->message[t_server->index] = t_server->c;
		t_server->index++;
		if (t_server->c == '\0')
		{
...
static void	msg_len_received(t_protocol *t_server)
{
	if (t_server->bits == sizeof(int) * 8 && t_server->flag == 0)
	{
		...
		ft_putnbr_fd(t_server->len, 1);
		ft_putstr_fd("]\n\e[0m", 1);
		t_server->message = calloc(t_server->len + 1, sizeof(char));
		...
		t_server->flag = 1;
		t_server->message[t_server->len] = '\0';
		t_server->bits = 0;
	}
}
...
static void	server_handler(int sig, siginfo_t *info, void *context)
{
	...
	if (sig == SIGUSR2 && t_server.flag == 0)
		t_server.len |= 1 << (((sizeof(int) * 8) - 1) - t_server.bits++);
	else if (sig == SIGUSR2 && t_server.flag == 1)
		t_server.c |= 1 << (((sizeof(char) * 8) - 1) - t_server.bits++);
	t_server.bits++;
